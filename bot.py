...
def paginate_ayahs(ayahs, page=0, page_size=20):
...
async def show_surah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        keyboard.append([
            ...
            InlineKeyboardButton("📖 عرض السورة كاملة (رسائل)", callback_data=f'fullsurah_{surah_num}'),
            InlineKeyboardButton("📄 تحميل السورة كملف", callback_data=f'surahfile_{surah_num}'),
...
# 📖 إرسال السورة كاملة على دفعات
async def show_full_surah(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        query = update.callback_query
        await query.answer()
        surah_num = query.data.split("_")[2]
        ...
        surah_data = response.json().get("data", {})
        ayahs = surah_data.get("ayahs", [])
        surah_name = surah_data.get("name", "السورة")

        if not ayahs:
            await query.edit_message_text("⚠️ لا توجد آيات في هذه السورة")
            return

        # تقسيم الآيات كل 20 آية لكل رسالة
        for i in range(0, len(ayahs), 20):
            part = ayahs[i:i+20]
            text = f"<b>سورة {surah_name}</b> - آيات {i+1} إلى {i+len(part)}\n\n"
            text += "\n".join([f"{a['numberInSurah']}. {a['text']}" for a in part])
            await query.message.reply_text(text, parse_mode="HTML")

    except Exception as e:
        logger.error(f"Error in show_full_surah: {e}")

# 📄 إرسال السورة كملف نصي
async def send_surah_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        query = update.callback_query
        await query.answer()
        surah_num = query.data.split("_")[2]
        ...
        content = f"سورة {surah_name}\n\n"
        for ayah in ayahs:
            content += f"{ayah['numberInSurah']}. {ayah['text']}\n"

        file_path = f"{surah_name}.txt"
        ...
        await context.bot.send_document(
            ...
            filename=file_path,
            caption=f"📄 السورة كاملة: {surah_name}"
        )

    except Exception as e:
        logger.error(f"Error in send_surah_file: {e}")
